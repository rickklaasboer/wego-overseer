...
export default class GuildRepository implements BaseRepository<Guild> {
     ...
     */
    public async getById(id: string | number): Promise<Maybe<Guild>> {
     ...
     * Update a guild
     */
    public async update(data: Partial<Guild>): Promise<Guild> {
        const result = await Guild.query().updateAndFetch(data);
        return result;
    }
