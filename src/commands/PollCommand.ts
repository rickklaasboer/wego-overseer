...
export const PollCommand = new Command<ChatInputCommandInteraction<CacheType>>(
    {
        name: 'poll',
        ...
        options: [
            ...
            },
            {
                type: APPLICATION_COMMAND_OPTIONS.STRING,
                name: 'vote-emojis',
                description: 'Comma-separated vote emojis (optional, default = ' + DEFAULT_VOTE_OPTIONS.join(',') + ')',
                required: false,
                min_length: 1,
                max_length: 256
            }
        ...
        run: async (interaction) => {
            ...
            const voteOptions = interaction.options.getString('vote-emojis')?.split(',') 
            ...
            for (let i = 0; i < voteOptions.length; i++) {
                ...
                emojis.push(interaction.client.emojis.cache.find(e => e.toString() === voteOptions[i])?.id ?? voteOptions[i]);
            }
            ...
            if (interaction.options.getString('description')) {
                question.setDescription(`${interaction.options.getString('description')}`)
            }
            ...
            if (failedReactions.length > 0) {
                await interaction.followUp({
                    content: 'Whoops! Some reactions failed: ' + failedReactions.join(','),
                    ephemeral: true
                });
            }
        },
