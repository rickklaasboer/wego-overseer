...
export default class KarmaUserGraphCommand extends BaseInternalCommand {
    ...
    private createChart(clusters: Row[]) {
        const chart = {
            width: config.karma.graph.width,
            height: config.karma.graph.height,
            c: {
                type: 'line',
                data: {
    ...
    public async execute(interaction: DefaultInteraction): Promise<void> {
        try {
            ...
            const clusters = (await knex
                .table('karma')
                .select(
                    knex.raw(
                        "DATE_FORMAT(createdAt, '%x-%v') AS week, SUM(amount) AS amount, guildId, userId",
                    ),
                )
            ...
            const response = await this.graphService.getGraph(
                this.createChart(clusters),
            );
            const embed = new EmbedBuilder()
                .setTitle(trans('commands.karma.graph.get.title'))
                .setDescription(
                    trans(
                        ...
                        clusters
                            .reduce(
                                (sum, current) =>
                                    Number(sum) + Number(current.amount),
                                0,
                            )
                            .toString(),
                    ),
